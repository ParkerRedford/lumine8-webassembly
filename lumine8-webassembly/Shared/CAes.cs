...
namespace lumine8.Aes
{
    public class CAes
    {
        ...
        public CAes(string Username, string Password)
        {
            this.username = Username;
            this.password = Password;

            using SHA512 sha = SHA512.Create();
            iv = sha.ComputeHash(Encoding.UTF8.GetBytes(Username)).Take(16).ToArray();
            key = sha.ComputeHash(Encoding.UTF8.GetBytes(Password)).Take(16).ToArray();
        }
